...
namespace Pulsar {
    ...
    asmFunc SyncSelectedMiiToNetwork() {
        ASM(
            nofralloc;
            // Copy the newly selected Mii into the "local player" Mii group. Said Mii group
            // is what the game uses for sending Miis over the net.
            ...
            lis     r12, CopyMiiToOtherGroup__8MiiGroupFR8MiiGroupUcUc@h;
            ori     r12, r12, CopyMiiToOtherGroup__8MiiGroupFR8MiiGroupUcUc@l;
            mtctr   r12;
            bctrl;

            // I'm not sure if doing this is necessary, but most code I've seen in the vanilla game
            // calls this function. Seems to be related to displaying the Mii model?
            lis     r12, sInstance__10SectionMgr@ha;
            lwz     r3, sInstance__10SectionMgr@l(r12);
            lwz     r3, 0x98(r3);
            addi    r3, r3, 0x238;
            ...
            lis     r12, func_805FA940@h;
            ori     r12, r12, func_805FA940@l;
            mtctr   r12;
            bctrl;

            // Force updating the USER packet after selecting the new Mii.
            // (sets USERPacketHandler->isInitialized to false)
            // This forces the game into re-creating the outgoing USER packet again,
            // applying the newly selected Miis over.
    ...
    // If the online countdown times out while idling in Mii Select,
